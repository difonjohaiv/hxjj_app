...
class DocRetrieve:
    def __init__(self, top_k: int = 15, txt_folder: str = PDF_TXT_PATH):
        self.top_k = top_k
        self.txt_folder = txt_folder
        self.doc_dict = dict()
        self.company_list = []
        self.get_all_company()  # 初始化公司名字，根据txt
    ...
    def search(self, company, query, score_func=bm25_retrieve):
        ...
        return "\n\n".join(
            [
                f"检索内容{i+1}开始:\n. {retrieve_content[0]}\n检索内容{i+1}结束."
                for i, retrieve_content in enumerate(
                    score_func(query, contents)[: self.top_k]
                )
            ]
        )

    def get_single_extra_knowledge(self, company, query):
        company, question = self.check_company(company, query)  # 公司名，用户问题
