...
class DocumentRetrieveChain(Tool):
    ...
    parameters: list = [{"name": "company", "description": "公司", "required": False}]

    def _local_call(self, *args, **kwargs):
        extra_knowledge = doc_retrieve.get_single_extra_knowledge(
            kwargs.get("company", ""), args[0]
        )  # 获取公司名，用户问题
        user_input = BS_CHAT_KNOWLEDGE_TEMPLATE.replace(
            "{user_question}", args[0]
        ).replace("{extra_knowledge}", extra_knowledge)
