...
BS_TASK_DEFAULT_PROMPT = (
    "你是一名高级智能助手，你需要根据当前提供的信息，执行当前任务。"
)
...
BS_SQL_GENERATOR_TEMPLATE_1 = (
    "你是一名sqlite数据库开发人员，精通sql语言，你需要根据已知的10张表的表名、字段名和用户输入的问题编写sql\n\n"
...
class BSPromptGenerator(PromptGenerator):
    ...
    def generate(self, task_no=None):
        # init plan 没有明确任务，初始化公共prompt
        if task_no is None:
            prompt_list = [self.system_prompt, self.user_prompt]
        # execute tasks
        else:
            # no task result
